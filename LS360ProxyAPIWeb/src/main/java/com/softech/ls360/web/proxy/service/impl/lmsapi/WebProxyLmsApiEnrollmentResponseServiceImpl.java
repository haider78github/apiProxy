...
@Service
public class WebProxyLmsApiEnrollmentResponseServiceImpl implements WebProxyLmsApiEnrollmentResponseService {
	...
	public LearnerEnrollmentsResponse processLearnerCoursesEnrollResponse(RegisterUser registerUser,
		...
		if (!CollectionUtils.isEmpty(enrolledCourseSuccessful)) {
			...
			List<Enrollments> enrollmentsList = lmsProxyEnrollmentsDao.findEnrollmentsByUserNameAndCoursesGuid(userName, coursesGuidList);
			Map<Long, String> enrollmentsMap = getEnrollmentsMap(enrollmentsList);
	...
	private Map<Long, String> getEnrollmentsMap(List<Enrollments> enrollmentsList) throws Exception {
		
		Map<Long, String> enrollmentIdToGuidMap = null;
		if (!CollectionUtils.isEmpty(enrollmentsList)) {
			enrollmentIdToGuidMap = enrollmentsList
					.stream()
					.collect(Collectors.toMap(Enrollments::getEnrollmentId, Enrollments::getProductId));
		}
		
		return enrollmentIdToGuidMap;
		
	}
	
	private List<LmsApiEnrolledCourse> getLmsApiEnrolledCoursesList(Map<Long, String> enrollmentsMap) {
	...
	public List<LmsApiEnrolledCourse> getLmsApiEnrolledCoursesList(List<EnrolledCourse> lmsApiEnrolledCoursesUnsuccessful) throws Exception {
		...
		if (!CollectionUtils.isEmpty(lmsApiEnrolledCoursesUnsuccessful)) {
			lmsApiEnrolledCoursesList  = lmsApiEnrolledCoursesUnsuccessful
					.stream()
					.map(p -> getLmsApiEnrolledCourse(0L, p.getCourseId(), p.getErrorCode(), p.getErrorMessage()))
					.collect(Collectors.toList());
		}
	...
	private LmsApiEnrolledCourse getLmsApiEnrolledCourse( Long enrollmentId, String courseGuid, String errorCode, String errorMessage) {
