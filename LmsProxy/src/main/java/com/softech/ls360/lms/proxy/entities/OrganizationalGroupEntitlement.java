...
@Entity
public class OrganizationalGroupEntitlement extends BaseEntity implements Serializable {
...
private static final long serialVersionUID = 1L;
	...
	private Integer allowUnlimitedEnrollmentsTf;
	 ...
	 * 
	 * Consider the ORGANIZATIONALGROUPENTITLEMENT and ORGANIZATIONALGROUP tables. The ORGANIZATIONALGROUPENTITLEMENT table has a foreign key column
	 * named ORGANIZATIONALGROUP_ID that references the ORGANIZATIONALGROUP table. From the perspective of the entity relationship, COURSEGROUP_ID is the join
	 * column that associates the OrganizationalGroupEntitlement and OrganizationalGroup entities.
	 * 
	 ...
	 * name element is used. For example, the @JoinColumn(name="ORGANIZATIONALGROUP_ID") annotation means that the ORGANIZATIONALGROUP_ID column in the 
	 ...
	 * relationship attribute in the source entity, which is "organizationalGroup" here, plus an underscore character (_), plus the
	 * name of the primary key column of the target entity. So if the OrganizationalGroup entity were mapped to a table that had a primary 
	 * key column named ID, the join column in the ORGANIZATIONALGROUPENTITLEMENT table would be assumed to be named ORGANIZATIONALGROUP_ID. If 
	...
	public Integer getAllowUnlimitedEnrollmentsTf() {
		return allowUnlimitedEnrollmentsTf;
	}
	
	public void setAllowUnlimitedEnrollmentsTf(Integer allowUnlimitedEnrollmentsTf) {
		this.allowUnlimitedEnrollmentsTf = allowUnlimitedEnrollmentsTf;
	}
